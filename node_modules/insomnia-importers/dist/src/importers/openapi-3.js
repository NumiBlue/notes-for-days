...
const getDefaultServerUrl = (api) => {
    ...
    if (!foundServer) {
        return (0, url_1.parse)(exampleServer);
    }
...
const parseDocument = (rawData) => {
    try {
        return ((0, utils_1.unthrowableParseJson)(rawData) ||
 ...
 */
const isSpecExtension = (property) => {
...
const parseEndpoints = (document) => {
    ...
    const endpointsSchemas = Object.keys(document.paths)
        ...
        const methods = Object.entries(schemasPerMethod)
            ...
            .filter(([key, value]) => (0, ramda_adjunct_1.isPlainObject)(value) && !isSpecExtension(key));
...
const parseSecurity = (security, securitySchemes) => {
    ...
    const apiKeyHeaders = apiKeySchemes
        ...
        .map(scheme => {
        const variableName = (0, change_case_1.camelCase)(scheme.name);
        return {
            name: scheme.name,
            disabled: false,
            value: `{{ ${variableName} }}`,
...
const getSecurityEnvVariables = (securitySchemeObject) => {
    ...
    const apiKeyVariableNames = securitySchemes
        ...
        .map(scheme => (0, change_case_1.camelCase)(scheme.name));
    const variables = {};
